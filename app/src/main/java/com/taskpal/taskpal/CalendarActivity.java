...
public class CalendarActivity extends AppCompatActivity implements EasyPermissions.PermissionCallbacks {
    ...
    static final String TASK_PAL_EVENT_SIGNATURE = "Event generated by Task Pal.";
    ...
    void calculateSchedule(String[] task) {
        ...
        Event new1 = new Event().setDescription(TASK_PAL_EVENT_SIGNATURE)
                                .setSummary(task[0])
                                .setLocation(task[1]);
        ...
        for (Event event : events) {
            ...
            // Implement after it's clear how to convert DateTime to Calendar date type
            // EventDateTime eventStart = event.getStart();
            // long eventDate = eventStart.getDate().getValue();
            // NotificationScheduler.setReminder(CalendarActivity.this, AlarmReceiver.class, event.getStart());
        }
    }
    ...
    // Notifications
    public void scheduleNotification(String taskName, long time, PendingIntent pendingIntent) {
        AlarmManager am = (AlarmManager) getApplicationContext().getSystemService(ALARM_SERVICE);
        am.set(AlarmManager.RTC_WAKEUP, time, pendingIntent);
    }

    /**
     * Fetches next task event.
     * @return Event object for task. Null if no task event is found in next 20 events.
     * @throws IOException
     */
    private Event getUpcomingTask() throws IOException {
        // List the next 10 events from the primary calendar.
        DateTime now = new DateTime(System.currentTimeMillis());
        List<String> eventStrings = new ArrayList<String>();
        Events events = service.events().list("primary")
        ...
        Event scheduledEvent;
        for (Event event : items) {
            if (event.getDescription().contains(TASK_PAL_EVENT_SIGNATURE)) {
                return event;
            }
        }
        return null;
